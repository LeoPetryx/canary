...
void EventCallback::creatureOnCombat(const std::shared_ptr<Creature> &attacker, const std::shared_ptr<Creature> &target, CombatDamage &damage) const {
...
void EventCallback::playerOnThink(const std::shared_ptr<Player> &player, uint32_t interval) const {
	if (!getScriptInterface()->reserveScriptEnv()) {
