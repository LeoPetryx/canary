...
public:
	...
	void creatureOnCombat(const std::shared_ptr<Creature> &attacker, const std::shared_ptr<Creature> &target, CombatDamage &damage) const;
	...
	void playerOnThink(const std::shared_ptr<Player> &player, uint32_t interval) const;
